...
export function useOnboardingActions() {
  ...
  return {
    updateCase: useMutation({
      mutationFn: ({ id, ...input }: { id: string } & Parameters<typeof onboardingApi.updateOnboardingCase>[1]) =>
    ...
    addTask: useMutation({
      mutationFn: ({ caseId, ...input }: { caseId: string } & Parameters<typeof onboardingApi.addOnboardingTask>[1]) =>
        onboardingApi.addOnboardingTask(caseId, input),
      onSuccess: invalidate,
    }),
    updateTask: useMutation({
      mutationFn: ({ taskId, ...input }: { taskId: string } & Parameters<typeof onboardingApi.updateOnboardingTask>[1]) =>
